import numpy as np
import torch
from ImageTools.c_interface import interp_c_bilinear, interp_c_nearest
...
def np_warp_im_bilinear(im, transform, new_im_size):
    ...
    old_r = old_coords[1]
    old_c = old_coords[0]
    pixel_vals = interp_c_bilinear(im, old_r.flatten(), old_c.flatten())
    im_c = pixel_vals.reshape(list(new_im_size)+ [im.shape[-1]])
    # im_np =  np_interpolate_image(im, old_r, old_c)
    return im_c

def np_warp_im_nearest(im, transform, new_im_size):
    rNew = np.arange(new_im_size[0]).reshape(-1,1).repeat(new_im_size[1], 1)
    cNew = np.arange(new_im_size[1]).reshape(1,-1).repeat(new_im_size[0], 0)
    inv_mapping = np.linalg.inv(transform.A)
    trans_inv = NpAffineTransforms(inv_mapping)
    ...
    old_r = old_coords[1]
    old_c = old_coords[0]
    pixel_vals = interp_c_nearest(im, old_r.flatten(), old_c.flatten())
    im_c = pixel_vals.reshape(new_im_size)
    # im_np =  np_interpolate_image(im, old_r, old_c)
    return im_c
